...
@Autonomous(name = "autoTest", group = "templates")
//@Disabled
public class BasicAuto extends LinearOpMode {
    ...
    public void circle(int diameter){
        L.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        R.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        L.setTargetPosition((int)Math.round(ENCODERCOUNT/CIR*diameter*12));
        L.setPower((Math.PI*12*(diameter+7))/2/(Math.PI*12*(diameter-7)));
        R.setPower(.5);
        while (L.isBusy()) {
        }
        L.setPower(0);
